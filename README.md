...
Основным различием систем master-slave и master-master является способ записи данных. Для master-slave данные записываются ТОЛЬКО через мастер, master-master позволяет писать на любой из серверов.

Различия удобно рассматривать в развере преимуществ и недостатков.

master-slave преимущества: невозможны конфликты данных, легкая настройка, хорошее ускорение работы по чтению

master-slave недостатки:  при отказе мастера запись невозможна без ручного вмешательства / до утсранения отказа


master-master преимущества: при отказе работоспособность полностью сохраняется, ускорение как по чтению так и по записи

master-master недостатки:  риск ошибок из-за (рас)синхронизации данных, более сложная настройка
